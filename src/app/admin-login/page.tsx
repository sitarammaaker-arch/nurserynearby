...
export default function AdminLoginPage() {
  ...
  // Lock after 5 failed attempts for 60s
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (newAttempts >= 5) {
          ...
          setError(`${data.error} (${5 - newAttempts} attempts remaining)`);
        }
        setPassword("");
        return;
      }
  ...
  return (
    <div className="min-h-screen gradient-sage flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        {/* Default password hint — REMOVE IN PRODUCTION */}
        {process.env.NODE_ENV !== "production" && (
          <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 text-center">
            <p className="text-xs text-amber-700 font-medium">🔑 Dev hint: default password is <code className="bg-amber-100 px-1 rounded font-mono">admin@nursery2024</code></p>
            <p className="text-2xs text-amber-600 mt-1">Set <code className="font-mono">ADMIN_PASSWORD</code> in .env to change it</p>
          </div>
        )}
      </div>
    </div>
